...
export const getCricMatchScorecard = async (req: Request, res: Response) => {
  try {
    const keyEntries = getCachedKeyEntries();
    ...
    for (const [index, key] of keyEntries.entries()) {
      try {
        const url = `${CRICAPI_BASE}/match_scorecard?apikey=${encodeURIComponent(
          key.value
        )}&id=${encodeURIComponent(id)}`;
    ...
    console.error("cricapi scorecard proxy error:", e);
    return res.status(502).json({ message: "Failed to reach CricAPI" });
  }
};
